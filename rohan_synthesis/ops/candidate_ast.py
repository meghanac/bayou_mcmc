...
from program_helper.ast.ops import CHILD_EDGE, Node, DAPIInvoke, DVarDecl, DClsInit, DVarAssign, DBranch, \
    ...
    MAJOR_CONCEPTS, CONTROL_FLOW_NAMES, SIBLING_EDGE
import numpy as np
...
CONCEPT_NODE, API_NODE, TYPE_NODE, VAR_NODE, OP_NODE, SYMTAB_MOD = 0, 1, 2, 3, 4, 5


class Candidate_AST:
    def __init__(self, initial_state, initial_symtab, ret_type, formal_params, field_types, prob):
    ...
    def remove_invalid(self, head):
        ...
        while node is not None:
            if node.val not in [DAPIInvoke.name(), DClsInit.name(), DVarDecl.name(), DExceptionVarDecl.name(),
                                DStop.name(),
                                DVarAssign.name(),
    ...
    def add_to_storage(self):
        ...
        self.storage.append(
            [self.curr_node_val, self.curr_edge, None,
             self.var_decl_id,
    ...
    def debug_print(self, vocab_types):
        print('\nType Helper || Expr type || Ret type')
