...
class DatasetCreator:
    ...
    def add_include_test_progs(self, added_to_test_set_func, category, novelty_label):
        ...
        def add_to_include(added_apis, added_pairs, counter, test_set, num_pairs_added, start_time,
            ...
            for api_idx in api_idx_range:
                ...
                # Double check
                if api in {DSUBTREE, DSTOP, DBRANCH, DEXCEPT, DLOOP}:
                    continue

                # Check if we can add duplicates to added_apis
                if api in added_apis and num_allow_of_each_api <= len(added_pairs[api]):
                    continue
                ...
                if category == IN_API:
                    ...
                    if num_allow_of_each_api == 1:
                        apis_with_api = list(apis_with_api - added_apis - {api, 'DSubTree', 'DStop', DBRANCH, DLOOP,
                                                                           DEXCEPT})
                    else:
                        apis_with_api = list(apis_with_api - added_pairs[api] - {api, 'DSubTree', 'DStop', DBRANCH, DLOOP,
                                                                           DEXCEPT})
                else:
                    apis_with_api = list(
                ...
                for dp2_idx in apis_with_api:
                    ...
                    # Double check
                    if api == dp2 or dp2 in {'DSubTree', 'DStop'} or dp2 in added_pairs[api]:
                        continue

                    # Check if more duplicates are allowed
                    if category in DP2_API and dp2 in added_apis and num_allow_of_each_api <= len(added_pairs[dp2]):
                        continue

                    progs_with_dp2 = self.ga.get_program_ids_for_api(dp2)
                    progs_with_both = progs_with_api.intersection(progs_with_dp2)

                    # If there are no programs of api and dp2 without each other, skip
                    if len(progs_with_both) == len(progs_with_api.union(progs_with_dp2)):
                        continue

                    # There needs to be enough programs in training set and more than 1 of this type in test set
                    if not (min_num_of_test_progs_per_pair <= len(progs_with_both) < len(progs_with_api) and
                            len(progs_with_dp2) > len(progs_with_both) >= min_num_of_test_progs_per_pair):
                        continue
                    ...
                    if counter % 5000 == 0:
                        ...
                        # If the progs meet the criteria
                        if added_to_test_set_func(api, data_point2, novelty_label, test_set, progs_with_api,
                                                  progs_with_dp2,
                                                  progs_with_both):
                            print(api, dp2)
                            num_pairs_added += 1
                            added_apis.add(api)
                            # add API to added_pairs
                            added_pairs[api].add(dp2)
                            if category in DP2_API:
                                added_apis.add(data_point2)

                                if dp2 not in added_pairs:
                                    added_pairs[dp2] = set([api])
                                else:
                                    added_pairs.add(api)
            ...
            return added_apis, added_pairs, counter, test_set, num_pairs_added

        added_apis, added_pairs, counter, test_set, num_pairs_added = add_to_include(added_apis, added_pairs, counter,
                                                                                     test_set, num_pairs_added,
                                                                                     start_time)
    ...
    def add_exclude_test_progs(self, category, novelty_label):
        ...
        def add_to_exclude(added_apis, added_pairs, counter, test_set, num_pairs_added, start_time,
            ...
            for api_idx in api_idx_range:
                ...
                if category == EX_API:
                    ...
                    if num_allow_of_each_api == 1:
                        ...
                        apis_with_api = list(
                            apis_with_api - added_pairs[api] - {api, 'DSubTree', 'DStop', DBRANCH, DLOOP,
                                                                DEXCEPT})
                else:
                    apis_with_api = list(
                        apis_with_api.intersection(set([self.ga.vocab2node[i] for i in [DBRANCH, DLOOP, DEXCEPT]])))
                ...
                for dp2_idx in apis_with_api:
                    ...
                    # Check if more duplicates are allowed
                    if category in DP2_API and num_allow_of_each_api <= len(added_pairs[dp2]):
                        continue

                    dp2_prog_ids = self.ga.get_program_ids_for_api(dp2)

                    # If all programs have both of them with each other, skip
                    ...
                    apis_with_both = list(set(apis_with_api).union(apis_with_dp2) - added_pairs[api] - {dp2, 'DSubTree', 'DStop'})
                    ...
                    for i3_idx in apis_with_both:
                        ...
                        # If there are not enough test progs, skip
                        progs_with_api_dp2 = api_prog_ids.intersection(dp2_prog_ids).difference(i3_prog_ids)
                        if len(progs_with_api_dp2) < min_num_of_test_progs_per_pair:
                            continue

                        # if i3 only appears with a cs structure, skip
                        ...
                        if api not in added_apis and not (category in DP2_API and dp2 in added_apis) and not (
                                category in DP2_API and i3 in added_apis):
                            ...
                            if success:
                                ...
                                added_apis.add(api)
                                added_pairs[api].add(dp2)
                                added_pairs[api].add(i3)

                                if category in DP2_API:
                                    added_apis.add(dp2)
                                    if dp2 in added_pairs:
                                        added_pairs[dp2].add(api)
                                        added_pairs[dp2].add(i3)
                                    else:
                                        added_pairs[dp2] = set([api, i3])

                                if i3 not in {DBRANCH, DLOOP, DEXCEPT}:
                                    added_apis.add(i3)
                                    if i3 in added_pairs:
                                        added_pairs[i3].add(api)
                                        added_pairs[i3].add(dp2)
                                    else:
                                        added_pairs[i3] = set([api, dp2])
        ...
        added_apis, added_pairs, counter, test_set, num_pairs_added = add_to_exclude(added_apis, added_pairs, counter,
                                                                                     test_set, num_pairs_added,
                                                                                     start_time)
    ...
    def create_curated_dataset(self):
        ...
        for novelty_label in [NEW]:  # Create novelty test set first
            ...
            print("INCLUDE API: ")
            start_time = time.time()
            self.add_include_test_progs(self.added_to_include_test_set, IN_API, novelty_label)
            ...
            print("INCLUDE CS: ")
            start_time = time.time()
            self.add_include_test_progs(self.added_to_include_test_set, IN_CS, novelty_label)
