...
from mcmc.mcmc import MCMCProgram
...
class Experiments:
    def __init__(self, data_dir_path, model_dir_path, num_iterations, save_mcmc_progs=True):
        self.metrics = Metrics(num_iterations * 1.0)
        self.num_iter = num_iterations * 1.0
        self.model_dir_path = model_dir_path

        training_path = data_dir_path + "/train/"
        testing_path = data_dir_path + "/test/"

        self.train_ga = GraphAnalyzer(training_path)
        self.all_test_ga = GraphAnalyzer(testing_path)  # TODO: fix
        self.test_ga = GraphAnalyzer(testing_path)
    ...
    def calculate_metrics(self, category, label):
        ...
        for posterior_dist, ast, ret_type, fp in post_dist_dict.values():
            ...
            jaccard_api_metric += jaccard_api
            jaccard_seq_metric += jaccard_seq
            ast_eq_metric += ast_eq
